import{F as e,I as t,M as n,N as r,P as i,et as a,j as o,k as s,rt as c,tt as l}from"./index-CwHsDQ2q.js";var u=`
      ...
      `).join(``),x=Object.entries(e.officialFields).filter(([t,n])=>s(t,e.request.inspectionType,!!(n??``).trim())).map(([e,t])=>`
        <tr>
...
${u}
      ...
      <body>
        ...
        <section>
          <h2>Findings</h2>
          ${i||`<p>No approved findings yet.</p>`}
        </section>
        ${p}
        <section>
...
${u}
  ...
  `}var h={satisfactory:`cond-ok`,marginal:`cond-marginal`,deficient:`cond-deficient`,not_present:`cond-na`,not_inspected:`cond-ni`};function g(a,s=!1){let d=a.request,p=a.property,m=[[`Client`,d.clientName],[`Phone`,d.phone],[`Email`,d.email],[`Property`,[p.address,p.city,p.state,p.postalCode].filter(Boolean).join(`, `)],[`Owner of record`,p.ownerName||``],[`Year built`,p.yearBuilt||``],[`Approx. area`,p.squareFeet?`${p.squareFeet} sq ft`:``],[`Occupancy`,p.occupancy||``],[`Inspection date`,a.inspectionDate||(d.appointmentStart?d.appointmentStart.slice(0,10):``)],[`Inspection type`,c(d.inspectionType)]].map(([e,t])=>`<div class="fact"><dt>${v(e)}</dt><dd>${v(t||`Not provided`)}</dd></div>`).join(``),g=t(a),y=g.length?g.map(e=>`
        <tr>
          <td>${v(e.systemLabel)}</td>
          <td>${v(e.componentLabel)}</td>
          <td><span class="cond ${h[e.condition]}">${v(o[e.condition])}</span></td>
          <td>${v(e.comment||`See system section.`)}</td>
        </tr>`).join(``):`<tr><td colspan="4">No deficiencies noted at the time of inspection.</td></tr>`,b=i.map(t=>{let n=t.components.map(t=>{let n=e(a,t.id);return`
          <tr>
            <td>${v(t.label)}</td>
            <td><span class="cond ${h[n.condition]}">${v(o[n.condition])}</span></td>
      ...
      <body>
        ...
        <section>
          ...
          <div class="sig">
            ...
            <div>
              <div class="line">Date — ${v(a.signedAt?l(a.signedAt):a.inspectionDate||``)}</div>
            </div>
          </div>
        </section>
      </body>
