import{A as e,F as t,M as n,N as r,O as i,P as a,et as o,j as s,rt as c,tt as l}from"./index-BbATE7zK.js";var u=`
      ...
      `).join(``),x=Object.entries(e.officialFields).filter(([t,n])=>i(t,e.request.inspectionType,!!(n??``).trim())).map(([e,t])=>`
        <tr>
          <td>${v(e.replace(/([A-Z])/g,` $1`))}</td>
          <td>${v(t||`Blank`)}</td>
        </tr>
      `).join(``),S=[[`Property`,`${e.property.address}, ${e.property.city}, ${e.property.state} ${e.property.postalCode}`],[`Owner`,e.property.ownerName||`Not populated`],[`County / parcel`,`${e.property.county||`Not populated`} / ${e.property.parcelId||`Not populated`}`],[`Legal description`,e.property.legalDescription||`Not populated`],[`Flood zone`,`${e.property.floodZone||`Not populated`}${e.property.sfha?` — SFHA ${e.property.sfha}`:``}`],[`Inspection date`,e.inspectionDate||`Not set`],[`Inspector`,`${e.inspector.name} — ${e.inspector.license}`],[`Company`,`${e.inspector.company||`Not set`} — ${e.inspector.email||`No email`}`],[`State pack`,`${t.name} ${t.version}`],[`Scope`,e.scope||`Not set`],[`Signoff`,e.signedAt?`${e.signatureName||e.inspector.name} at ${l(e.signedAt)}`:`Pending inspector finalization`]].map(([e,t])=>`<div class="fact"><dt>${v(e)}</dt><dd>${v(t)}</dd></div>`).join(``);return`
    <!doctype html>
    <html>
      <head>
        <meta charset="utf-8" />
...
${u}
      ...
      <body>
        ...
        <section>
          ...
          <p>Signed at: ${v(e.signedAt?l(e.signedAt):`Pending`)}</p>
          <p>Exported at: ${v(e.exportedAt?l(e.exportedAt):`Pending`)}</p>
        </section>
      </body>
    </html>
  `}function m(e,t=!1){let n=e.permitCandidates.map(e=>`
      ...
      `).join(``),r=[[`Property`,`${e.property.address}, ${e.property.city}, ${e.property.state} ${e.property.postalCode}`],[`Owner`,e.property.ownerName||`Not populated`],[`Parcel / tax acct`,`${e.property.parcelId||`—`} / ${e.property.taxAccount||`—`}`],[`Inspector`,`${e.inspector.name} — ${e.inspector.license}`],[`Prepared`,l(new Date().toISOString())]].map(([e,t])=>`<div class="fact"><dt>${v(e)}</dt><dd>${v(t)}</dd></div>`).join(``);return`
    <!doctype html>
    <html>
      <head>
        <meta charset="utf-8" />
...
${u}
  ...
  `}var h={satisfactory:`cond-ok`,marginal:`cond-marginal`,deficient:`cond-deficient`,not_present:`cond-na`,not_inspected:`cond-ni`};function g(i,o=!1){let d=i.request,p=i.property,m=[[`Client`,d.clientName],[`Phone`,d.phone],[`Email`,d.email],[`Property`,[p.address,p.city,p.state,p.postalCode].filter(Boolean).join(`, `)],[`Owner of record`,p.ownerName||``],[`Year built`,p.yearBuilt||``],[`Approx. area`,p.squareFeet?`${p.squareFeet} sq ft`:``],[`Occupancy`,p.occupancy||``],[`Inspection date`,i.inspectionDate||(d.appointmentStart?d.appointmentStart.slice(0,10):``)],[`Inspection type`,c(d.inspectionType)]].map(([e,t])=>`<div class="fact"><dt>${v(e)}</dt><dd>${v(t||`Not provided`)}</dd></div>`).join(``),g=t(i),y=g.length?g.map(t=>`
        <tr>
          <td>${v(t.systemLabel)}</td>
          <td>${v(t.componentLabel)}</td>
          <td><span class="cond ${h[t.condition]}">${v(e[t.condition])}</span></td>
          <td>${v(t.comment||`See system section.`)}</td>
        </tr>`).join(``):`<tr><td colspan="4">No deficiencies noted at the time of inspection.</td></tr>`,b=r.map(t=>{let n=t.components.map(t=>{let n=a(i,t.id);return`
          <tr>
            <td>${v(t.label)}</td>
            <td><span class="cond ${h[n.condition]}">${v(e[n.condition])}</span></td>
      ...
      <body>
        ...
        <section>
          ...
          <div class="sig">
            ...
            <div>
              <div class="line">Date — ${v(i.signedAt?l(i.signedAt):i.inspectionDate||``)}</div>
            </div>
          </div>
        </section>
      </body>
