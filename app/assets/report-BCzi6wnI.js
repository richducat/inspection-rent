import{A as e,E as t,M as n,N as r,O as i,et as a,j as o,k as s,rt as c,tt as l}from"./index-BbGPBzvL.js";var u=`
      ...
      `).join(``),S=[[`Property`,`${e.property.address}, ${e.property.city}, ${e.property.state} ${e.property.postalCode}`],[`Owner`,e.property.ownerName||`Not populated`],[`County / parcel`,`${e.property.county||`Not populated`} / ${e.property.parcelId||`Not populated`}`],[`Legal description`,e.property.legalDescription||`Not populated`],[`Flood zone`,`${e.property.floodZone||`Not populated`}${e.property.sfha?` — SFHA ${e.property.sfha}`:``}`],[`Inspection date`,e.inspectionDate||`Not set`],[`Inspector`,`${e.inspector.name} — ${e.inspector.license}`],[`Company`,`${e.inspector.company||`Not set`} — ${e.inspector.email||`No email`}`],[`State pack`,`${n.name} ${n.version}`],[`Scope`,e.scope||`Not set`],[`Signoff`,e.signedAt?`${e.signatureName||e.inspector.name} at ${l(e.signedAt)}`:`Pending inspector finalization`]].map(([e,t])=>`<div class="fact"><dt>${v(e)}</dt><dd>${v(t)}</dd></div>`).join(``);return`
    <!doctype html>
    <html>
      <head>
        <meta charset="utf-8" />
...
${u}
      ...
      <body>
        ...
        <section>
          <h2>Findings</h2>
          ${o||`<p>No approved findings yet.</p>`}
        </section>
        ${p}
        <section>
...
${u}
  ...
  `}var h={satisfactory:`cond-ok`,marginal:`cond-marginal`,deficient:`cond-deficient`,not_present:`cond-na`,not_inspected:`cond-ni`};function g(t,a=!1){let d=t.request,p=t.property,m=[[`Client`,d.clientName],[`Phone`,d.phone],[`Email`,d.email],[`Property`,[p.address,p.city,p.state,p.postalCode].filter(Boolean).join(`, `)],[`Owner of record`,p.ownerName||``],[`Year built`,p.yearBuilt||``],[`Approx. area`,p.squareFeet?`${p.squareFeet} sq ft`:``],[`Occupancy`,p.occupancy||``],[`Inspection date`,t.inspectionDate||(d.appointmentStart?d.appointmentStart.slice(0,10):``)],[`Inspection type`,c(d.inspectionType)]].map(([e,t])=>`<div class="fact"><dt>${v(e)}</dt><dd>${v(t||`Not provided`)}</dd></div>`).join(``),g=r(t),y=g.length?g.map(e=>`
        <tr>
          <td>${v(e.systemLabel)}</td>
          <td>${v(e.componentLabel)}</td>
          <td><span class="cond ${h[e.condition]}">${v(i[e.condition])}</span></td>
          <td>${v(e.comment||`See system section.`)}</td>
        </tr>`).join(``):`<tr><td colspan="4">No deficiencies noted at the time of inspection.</td></tr>`,b=o.map(e=>{let r=e.components.map(e=>{let r=n(t,e.id);return`
          <tr>
            <td>${v(e.label)}</td>
            <td><span class="cond ${h[r.condition]}">${v(i[r.condition])}</span></td>
            <td>${r.comment?v(r.comment):`<span class='muted'>—</span>`}</td>
          </tr>`}).join(``);return`
      ...
      <body>
        ...
        <section>
          ...
          <div class="sig">
            ...
            <div>
              <div class="line">Date — ${v(t.signedAt?l(t.signedAt):t.inspectionDate||``)}</div>
            </div>
          </div>
        </section>
      </body>
