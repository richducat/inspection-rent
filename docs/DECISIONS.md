...
| 2026-08-19 | The iOS shell shows the free signup again; pricing and checkout stay web-only | Free signup is allowed by the guideline; only paid flows are not | app commit 96bbeee |
| 2026-08-05 | The iOS shell hides every purchase and billing surface; the web app shows "managed on the web" inside it | Apple App Review 2026-08-04, guideline 3.1.1 | app commit a88baa0 |
| 2026-08-14 | Records CI checks syntax and the Dockerfile copy list on every push | An image had built successfully and died on boot because a `COPY lib/` line was missing | hip-records-api commit e8ac165 |
| 2026-08-05 | Records metering fails closed on any non-402 accounts response; a forged unsigned token can never pull during an outage | A break-it pass found free metered pulls on timeouts and forged tokens working during outages | hip-records-api commits bda5cfb, 6086388 |
...
| 2026-07-29 | Every records deploy bumps the `deploy` marker in `GET /health` | A fix shipped and nobody could tell whether the live build had it | hip-records-api commits 3f4d34a (07-29), abf7ab2 (08-01) |
| 2026-07-22 (commits 07-23 UTC) | Backend pricing: unlimited $98/mo, annual $980/yr, monthly $20 for 10, single $5; the $98/yr price retired as legacy. **The website still sells $98/yr; unresolved** | Pricing decision of that day | hip-accounts-api commits db93cb0, f4e1df1; issue #12 |
...
| 2026-07-06 | A real accounts backend (Node, Express, SQLite, bcrypt, JWT, Stripe) replaces the app's client-side login gate; production refuses to boot without a real admin password. (It started on `better-sqlite3`; the code now uses Node's built-in `node:sqlite`, which is why Node 22 is required) | The client-side gate was not a real auth or billing boundary | hip-accounts-api commits 9d64804, 35bfff0 |
